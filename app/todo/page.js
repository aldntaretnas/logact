...
function TodoHariIni() {
  ...
  async function handleAdd(e) {
    ...
    setAdding(true)
    await supabase.from('todos').insert([{ title: newTitle.trim(), date: today, time: newTime || null, completed: false }])
    setNewTitle('')
    setNewTime('')
    setAdding(false)
    fetchTodos(today)
  ...
  return (
    <div>
      <div className="flex items-center gap-2 mb-4">
        ...
        <p className="text-sm font-medium text-slate-600">{formatDate(today)}</p>
      </div>
      ...
      <form onSubmit={handleAdd} className="bg-white rounded-xl border border-slate-200 p-5 mb-6 transition-shadow hover:shadow-[0_8px_24px_rgba(30,58,138,0.55)]">
        <label className="block text-sm font-semibold text-slate-800 mb-3">Tambahkan Tugas Hari Ini</label>
  ...
  return (
    <div>
      <div className="flex items-center gap-2 mb-4">
        ...
        <p className="text-sm font-medium text-slate-600">{formatTomorrow(tomorrow)}</p>
      </div>
